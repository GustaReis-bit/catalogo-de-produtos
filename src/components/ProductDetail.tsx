...
export default function ProductDetail({ product }: Props) {
  ...
  const stockLabel =
    product.stock === 0
      ? { text: 'Out of Stock', cls: 'bg-red-50 text-red-600 border-red-100' }
      : product.stock <= 5
      ? { text: `Low Stock — ${product.stock} left`, cls: 'bg-amber-50 text-amber-700 border-amber-100' }
      : { text: `In Stock (${product.stock})`, cls: 'bg-green-50 text-green-700 border-green-100' };
    ...
    <div className="animate-fade-up" style={{ animationDelay: '0s', opacity: 0 }}>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
        ...
        <div className="flex flex-col">
          ...
          <span className={`self-start px-3 py-1 rounded-sm text-sm border font-medium mb-5 ${stockLabel.cls}`}>
            {stockLabel.text}
          </span>

          {/* Descrição */}
          <div className="border-t border-ink-100 pt-5 mb-6">
            <h2 className="text-xs font-medium text-ink-400 uppercase tracking-widest mb-2">Descrição</h2>
          ...
          <div className="border-t border-ink-100 pt-5 mb-8">
            <dl className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <dt className="text-ink-400 text-xs uppercase tracking-wide mb-0.5">ID</dt>
                <dd className="font-mono text-ink-700">#{product.id}</dd>
              </div>
              <div>
                <dt className="text-ink-400 text-xs uppercase tracking-wide mb-0.5">Adicionado</dt>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-auto">
            ...
            <button
              ...
              disabled={deleting}
              className={`flex-1 px-6 py-3 text-sm font-medium rounded-sm transition-colors ${
                confirmDelete
                  ? 'bg-red-600 text-white hover:bg-red-700'
                  : 'bg-ink-950 text-cream hover:bg-ink-800'
              } disabled:opacity-50 disabled:cursor-not-allowed`}
            >
              {deleting ? 'Deleting…' : confirmDelete ? 'Confirmar Deletar' : 'Deletar Produto'}
            </button>
          </div>

          {confirmDelete && !deleting && (
            <p className="text-xs text-red-500 mt-2 text-center animate-fade-in">
              Clique Novamente para Deletar Permanentemente o Produto.
              <button onClick={() => setConfirmDelete(false)} className="underline ml-1">Cancelar</button>
            </p>
          )}
        </div>
      </div>
