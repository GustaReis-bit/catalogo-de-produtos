  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`w-3.5 h-3.5 ${star <= Math.round(rating) ? 'text-amber-500' : 'text-ink-200'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
      ...
      <span className="text-xs text-ink-400 ml-0.5 font-mono">{rating.toFixed(1)}</span>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] overflow-hidden bg-parchment">
        ...
        />
        {/* Badge de categoria */}
        <span className="absolute top-2.5 left-2.5 px-2 py-0.5 bg-white/90 backdrop-blur-sm
        ...
        {product.stock === 0 && (
          <div className="absolute inset-0 bg-white/60 flex items-center justify-center">
            <span className="bg-ink-950 text-cream text-xs font-medium px-3 py-1">Out of Stock</span>
          </div>
        )}
        {isLowStock && (
          <span className="absolute top-2.5 right-2.5 px-2 py-0.5 bg-amber-500 text-white
      ...
      {/* Conteúdo */}
      <div className="p-4">
        <h3 className="font-display font-semibold text-ink-950 text-base leading-snug mb-1
